...
echo "$(date): Starting GDS to MAG using Magic /tmp/OpenRAM/miniconda/bin/magic"
...
gds flatten true
gds ordering true
gds read myconfig.gds
puts "Finished reading gds myconfig.gds"
load myconfig
...
EOF
magic_retcode=$?
echo "$(date): Finished ($magic_retcode) GDS to MAG using Magic /tmp/OpenRAM/miniconda/bin/magic"
exit $magic_retcode
