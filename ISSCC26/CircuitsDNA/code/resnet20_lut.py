...
def load_lut_table_csv(path: str) -> torch.Tensor:
    if not os.path.isfile(path):
        raise FileNotFoundError(f'LUT CSV 不存在: {path}')
    ...
    with open(path, 'r', newline='') as fp:
        ...
        if reader.fieldnames is None or not {'A', 'B', 'OUT'} <= set(reader.fieldnames):
            raise ValueError('LUT CSV 缺少列: 需要包含 A,B,OUT')
        count = 0
        for row in reader:
            a = int(row['A'])
            b = int(row['B'])
            ...
            if not (-128 <= a <= 127 and -128 <= b <= 127):
                raise ValueError(f'LUT 键超出 int8 范围: A={a}, B={b}')
            table[a + 128, b + 128] = out
            count += 1
    if count != 256 * 256:
        raise ValueError(f'LUT CSV 数据量异常: 期望 65536 行, 实际 {count}')
    return table
...
def load_qat_state(cfg: ResNet20LUTCfg) -> dict:
    if not cfg.qat_ckpt or not os.path.isfile(cfg.qat_ckpt):
        raise FileNotFoundError(f'找不到 QAT 权重: {cfg.qat_ckpt}')
